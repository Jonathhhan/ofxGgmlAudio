param(
	[string] $SmokePath = ".audio-runtime-smoke.json",
	[string] $OutputPath = "build\evidence\audio-runtime-evidence.json",
	[string] $Repo = "ofxGgmlAudio",
	[string] $Lane = "speech-recognition",
	[string] $CommitSha = "",
	[string] $WorkflowName = "audio-runtime-smoke",
	[string] $RunnerOs = "",
	[string] $Backend = "",
	[string] $ArtifactPath = "",
	[string] $Command = "",
...
function Get-ToolVersions {
	...
	$versions["producer"] = "write-audio-runtime-evidence.ps1"
...
if (!(Test-Path -LiteralPath $resolvedSmokePath -PathType Leaf)) {
	throw "Audio runtime smoke JSON was not found: $resolvedSmokePath"
}
...
if ($null -eq $summary) {
	throw "Audio runtime smoke JSON must include a Summary object. Run scripts\run-audio-runtime-smoke.ps1 -Json -SummaryOnly -OutputPath .audio-runtime-smoke.json first."
}
...
if ([string]::IsNullOrWhiteSpace($Command)) {
	$Command = "scripts/run-audio-runtime-smoke.ps1 -Json -SummaryOnly -OutputPath $ArtifactPath"
}
...
$evidence = [ordered] @{
	...
	producer = "write-audio-runtime-evidence.ps1"
	...
	subject_paths = @(
		"scripts/run-audio-runtime-smoke.ps1"
	)
	quality_report_path = $QualityReportPath
	example_name = "ofxGgmlAudioTranscriptionExample"
...
if ($PassThru) { Write-Output $content } else { Write-Host "Wrote Audio runtime Evidence Schema v1 wrapper: $resolvedOutputPath" }
exit 0
