...
function Clear-BuildDirIfCompilerCacheIsStale {
	...
	foreach ($line in Get-Content -LiteralPath $cacheFile) {
		...
		if (!(Test-Path -LiteralPath $compilerPath -PathType Leaf)) {
			Write-Step "Cleaning stale Whisper chunked smoke CMake cache"
...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofa-chunk"
}
...
if ($DryRun) {
	Write-Step "Whisper chunked transcription smoke plan"
...
if (Test-WindowsHost) {
	...
	$configure = "cmake -S $(Convert-ToCmdArgument $testsDir) -B $(Convert-ToCmdArgument $BuildDir) -G $(Convert-ToCmdArgument "NMake Makefiles") -DCMAKE_BUILD_TYPE=$Configuration -DOFXGGMLAUDIO_BUILD_CHUNKED_WHISPER_SMOKE=ON -DOFXGGMLAUDIO_WHISPER_INCLUDE_DIR=$(Convert-ToCmdArgument $whisperInclude) -DOFXGGMLAUDIO_GGML_INCLUDE_DIR=$(Convert-ToCmdArgument $ggmlInclude) -DOFXGGMLAUDIO_WHISPER_LIBRARY=$(Convert-ToCmdArgument $whisperLib)"
	$build = "cmake --build $(Convert-ToCmdArgument $BuildDir) --target audio_chunked_smoke"
	$copyDll = "copy /Y $(Convert-ToCmdArgument $whisperDll) $(Convert-ToCmdArgument $BuildDir) >nul"
	$exe = Join-Path $BuildDir "audio_chunked_smoke.exe"
	...
	Write-Step "Building and running Whisper chunked transcription smoke with Visual Studio tools"
	Invoke-CheckedCmd "Whisper chunked transcription smoke" $command
} else {
	Write-Step "Configuring Whisper chunked transcription smoke"
	Invoke-CheckedNative "cmake configure Whisper chunked transcription smoke" {
		cmake -S $testsDir -B $BuildDir -DCMAKE_BUILD_TYPE=$Configuration `
			-DOFXGGMLAUDIO_BUILD_CHUNKED_WHISPER_SMOKE=ON `
	...
	Write-Step "Building Whisper chunked transcription smoke"
	Invoke-CheckedNative "cmake build Whisper chunked transcription smoke" {
		cmake --build $BuildDir --target audio_chunked_smoke --config $Configuration
	}
	Write-Step "Running Whisper chunked transcription smoke"
	$exe = Join-Path $BuildDir "audio_chunked_smoke"
	Invoke-CheckedNative "Whisper chunked transcription smoke" {
